...
class T(AffineTransform):
    ...
    def rotate_crop(self, angle, center, winsize, resample=Image.BILINEAR):
        ...
        corners = np.array(
            [[0, 0], [self.size[0], 0], [0, self.size[1]], [self.size[0], self.size[1]]]
        ).transpose()
...
def transform_2d_points(A, x, transpose=False, inverse=False):
    ...
    (dim, N) = x.shape
    assert dim == 2, "transform_2d_points: Only 2D points are supported, get " + str(
        dim
    )
...
def transform_pose_sequence(A, poses, inverse=True):
    ...
    assert (
        len(poses.shape) == 3
    ), "transform_pose_sequence: expected 3D tensor, got " + str(poses.shape)
...
def normalize_channels(frame, channel_power=1):
    ...
    if type(channel_power) is not int:
        assert len(channel_power) == 3, (
            "channel_power expected to be int or "
